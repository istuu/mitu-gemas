...
use App\Webarq\Model\ExchangeCodeModel as Model;
...
class ExchangeDatasController extends BaseController
{
    ...
    public function actionGetIndex(){
        $this->layout->{'rightSection'} = view('panel.exchange.all');
    }

    public function actionAjaxGetData(Request $request){
        $model = new Model;
        if($request->start !== ''){
            $start = date('Y-m-d H:i:s', strtotime(request()->start));
            $model = $model->where('create_on','>=',$start);
        }
        if($request->end !== ''){
            $end   = date('Y-m-d H:i:s', strtotime(request()->end));
            $model = $model->where('create_on','<=',$end);
        }
        $tables = Table::of($model->get())->make(true);
        return $tables;
    }
